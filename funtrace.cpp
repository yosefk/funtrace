...
#define FUNTRACE_LOG_BUF_SIZE 14
#endif

enum { FUNTRACE_BUF_SIZE = 1 << FUNTRACE_LOG_BUF_SIZE };
...
struct trace_data
{
    ...
    inline void NOINSTR pause_tracing() { pos = nullptr; }
    inline void NOINSTR resume_tracing() { pos = buf; }

    void NOINSTR allocate()
    {
        //allocate an aligned buffer for trace data
...
static inline void NOINSTR trace(void* ptr, uint64_t is_ret)
{
    ...
    trace_entry* entry = g_thread_trace.pos;
    if(!entry) {
        return;
    }
