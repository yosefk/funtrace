# `info proc mappings` format:
#         Start Addr           End Addr       Size     Offset objfile
#     0x555555554000     0x555555556000     0x2000        0x0 /path/to/file
#
# /proc/self/maps format - start-end permissions offset device inode /path/to/file 
# 7f74a4ae6000-7f74a4b08000 r--p 00000000 103:07 109578392                 /usr/lib/x86_64-linux-gnu/libc-2.31.so

import gdb, struct, traceback

def write_chunk(f, magic, content):
    assert len(magic)==8
    f.write(magic)
    f.write(struct.pack('Q', len(content)))
    f.write(content)

def write_proc_maps(f):
    mappings = gdb.execute('info proc mappings', from_tty=False, to_string=True)
 
    proc_maps = b''
    for line in mappings.strip().split('\n'):
        line = line.strip()
        if line.startswith('0x'):
            t = line.split()
            if len(t) == 5: # we don't care about unnamed segments
                start, end, size, offset, path = line.split()
                # we don't care about permissions, device and inode
                proc_maps += b'%10x-%10x r-xp %08x 0:0 0 %s\n'%(int(start,16), int(end,16), int(offset,16), bytes(path,encoding='utf-8'))
 
    print('funtrace: saving proc mappings')
    write_chunk(f, b'PROCMAPS', proc_maps)

def get_vector_elements(v):
    vis = gdb.default_visualizer(v)
    if vis:
        return [elem for _,elem in list(vis.children())]
    else: # no pretty printers - assume we know the representation
        start = v['_M_impl']['_M_start']
        finish = v['_M_impl']['_M_finish']

        return [start[i] for i in range(finish-start)] 

def get_string(s):
    vis = gdb.default_visualizer(s)
    if False and vis: # the False comments it out since we get garbage like "<repeats 11 times>" with this at times,
        # and I haven't found a way to avoid this
        return str(vis.to_string().value())[1:-1]
    else: # rely on the representation
        length = int(s['_M_string_length'])
        data_ptr = s['_M_dataplus']['_M_p']

        return str(gdb.selected_inferior().read_memory(data_ptr, length), encoding='utf-8')
        return str(gdb.Value(data_ptr.cast(gdb.lookup_type('char').pointer()).string(length=length)))[1:-1]

def write_ftrace(f):
    try:
        handler = gdb.parse_and_eval('g_ftrace_handler')
    except:
        print("funtrace: compiled without ftrace support - can't fetch last ftrace events")
        return

    events = get_vector_elements(handler['events'])
    pos = int(handler['pos'])
    lines = []
    def collect_lines(start, end):
        lines.extend([get_string(events[i]['line']) for i in range(start, end) if int(events[i]['timestamp'])])
    collect_lines(pos, len(events)) # these are the older ones
    collect_lines(0, pos)

    print(f'funtrace: saving {len(lines)} ftrace events')
    write_chunk(f, b'FTRACETX', ('\n'.join(lines) + '\n').encode('utf-8'))

def write_funtrace(f):
    p_trace_state = gdb.parse_and_eval('g_p_trace_state')
    if p_trace_state == gdb.Value(0).cast(p_trace_state.type):
        print('funtrace not initialized yet - no trace data')
        return
    trace_state = p_trace_state.dereference()

    write_chunk(f, b'FUNTRACE', struct.pack('Q', int(trace_state['cpu_freq'])))

    cmdline = get_string(trace_state['cmdline'])
    print(f'funtrace: core dump generated by `{cmdline}`')
    write_chunk(f, b'CMD LINE', cmdline.encode('utf-8'))

    thread_traces = get_vector_elements(trace_state['thread_traces'])
    for i, trace in enumerate(thread_traces):
        # separately add ftrace
        trace = trace.dereference()
        thread_id = trace['id']
        buf_size = trace['buf_size']
        write_chunk(f, b'THREADID', bytes(gdb.selected_inferior().read_memory(thread_id.address, thread_id.type.sizeof)))

        buf = trace['buf']
        data = bytes(gdb.selected_inferior().read_memory(buf, buf_size))
        name = thread_id["name"].string()
        print(f'funtrace: thread {thread_id["tid"]} {name} - saving {buf_size} bytes of data read from {buf}')
        write_chunk(f, b'TRACEBUF', data)

    write_ftrace(f)

    write_chunk(f, b'ENDTRACE', b'')
    print('funtrace: done - decode with `funtrace2viz funtrace.raw out` and then view in viztracer (pip install viztracer) with `vizviewer out.json`')

class FuntraceCmd(gdb.Command):
    '''prints the content of the funtrace event buffers into ./funtrace.raw

you can then decode that file using funtrace2viz and open the output JSON files with vizviewer
(installed by `pip install viztracer`) or Perfetto (https://ui.perfetto.dev, click
"Open with legacy UI" - no source access unlike in vizviewer but otherwise should work)'''

    def __init__(self):
        super(FuntraceCmd, self).__init__("funtrace", gdb.COMMAND_DATA)

    def invoke(self, arg, from_tty):
        try:
            with open('funtrace.raw', 'wb') as f:
                write_proc_maps(f)
                write_funtrace(f)
        except:
            traceback.print_exc()
            raise

FuntraceCmd()
